...
const PickupSuburbField = ({ setPickupSuburb, pickupSuburb, errors }) => {
  ...
  const renderSuggestion = (suggestion) => {
    return (
      <div className="flex items-center gap-3 px-4 py-3 hover:bg-blue-50 transition-colors duration-150 cursor-pointer">
        <svg 
          className="w-4 h-4 text-[#193D5A] flex-shrink-0" 
          fill="none" 
          stroke="currentColor" 
          viewBox="0 0 24 24"
        >
  ...
  const inputProps = {
    ...
    onChange: onChange,
    onBlur: onBlur,
    className: `w-full px-4 py-3 rounded-xl border-2 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-[#193D5A] ${
      errors?.pickupSuburb 
        ? 'border-red-300 bg-red-50 focus:border-red-500' 
        : 'border-gray-200 focus:border-[#193D5A]'
    }`,
  };
